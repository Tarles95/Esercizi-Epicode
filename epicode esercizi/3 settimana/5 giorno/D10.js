...
function splitMe(stringa) {
  var arrayStr = stringa.split(' ');

  return arrayStr;
}

var stringaSplit = "I love coding";
var risultato = splitMe(stringaSplit);
console.log(risultato);
...
*/
function deleteOne(stringa2, boolean) {
  // Verifica della condizione booleana
  if (boolean = true) {
    // Se la condizione è vera, ritorna la stringa senza il primo carattere
    return stringa2.slice(1);
  } else {
    // Se la condizione è falsa, ritorna la stringa senza l'ultimo carattere
    return stringa2.slice(0, -1);
  }
}

var inputString = "I love coding";


var esito = deleteOne(inputString);
console.log(esito);
...
function onlyLetters(stringa3) {
  var noNum = stringa3.replace(/[0-9]/g, '')
  return noNum;
}
inputStringDue = "I eat 1234 apples"
var esito2 = onlyLetters(inputStringDue);
console.log(esito2)
...
function whatDayIsIt() {
  ...
  var trovaGiorno = oggi.getDay();

  return giorniSettimana[trovaGiorno];
}

var giornoCorrente = whatDayIsIt();
...
const rolltheDices = function (sommaValori) {
  
  for(index=0; index<sommaValori; index++) {
    const arrayValues = dice();
    object.sum += arrayValues;
    object.values.push(arrayValues);
  }
  return object;
  }
...
function isTodayMyBirthday(mioCompleanno) {
  ...
  var giornoCompleanno = mioCompleanno.getDate();
  var meseCompleanno = mioCompleanno.getMonth() + 1; 
...
var mioCompleanno = new Date("1995-09-21"); 

var oggiIlMioCompleanno = isTodayMyBirthday(mioCompleanno);
...
function deleteProp ( obj, stringa5) {
...
console.log('Esercizio 12',filmPiuRecente)
...
var numeroFilm = countMovies(movies);
...
/*function sumAllTheYears(movies) {
  if (!movies || !Array.isArray(movies)) {
    return 0; // Restituisci 0 se l'input non è definito o non è un array
  }

  // Utilizza il metodo reduce per calcolare la somma degli anni di produzione
  var sommaAnni = movies.reduce(function(acc, movie) {
    var annoProduzione = new Date(movie.releaseDate).getFullYear();
    return acc + annoProduzione;
  }, 0);

  return sommaAnni;
}
var sommaAnni = sumAllTheYears(movies);
...
*/
function searchAndDivide(pippo){
   let match = [];
   let unmatch = [];
  ...
  if(titolo.includes(pippo)) {
    match.push(risultatoTrovato);
  } else {
    unmatch.push(risultatoTrovato);
  }
}
let oggetto = {nome1: match, nome2: unmatch};
return oggetto;
}
